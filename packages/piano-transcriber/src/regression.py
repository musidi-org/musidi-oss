...
def get_binarized_output_from_regression(reg_output, threshold, neighbour):
    ...
    def is_monotonic_neighbour(x, n, neighbour):
        ...
        for i in range(neighbour):
            if x[n - i] < x[n - i - 1] or x[n + i] < x[n + i + 1]:
                return False
        return True

    reg_output = np.array(reg_output, dtype=np.double)
    binary_output = np.zeros_like(reg_output, dtype=np.double)
    shift_output = np.zeros_like(reg_output, dtype=np.double)
    (frames_num, classes_num) = reg_output.shape

    neighbourLoop = range(neighbour, frames_num - neighbour)
    for k in range(classes_num):
        x = reg_output[:, k]
        for n in neighbourLoop:
        ...
        with Pedals by Regressing Onsets and Offsets Times, 2020."""
                if x[n - 1] > x[n + 1]:
                    shift = 0.5 * (x[n + 1] - x[n - 1]) / (x[n] - x[n + 1])
                else:
                    shift = 0.5 * (x[n + 1] - x[n - 1]) / (x[n] - x[n - 1])
...
class RegressionPostProcessor(object):
    def __init__(
        ...
        # MIDI note A0, lowest piano note
        self.begin_note = 21
        # MIDI max velocity
    ...
    def output_dict_to_note_pedal_arrays(self, output_dict):
        ...
        (onset_output, onset_shift_output) = get_binarized_output_from_regression(
        ...
        (offset_output, offset_shift_output) = get_binarized_output_from_regression(
    ...
    offset_shift and normalized_velocity"""
        ...
        if len(est_tuples) == 0:
            ...
            est_on_off_note_vels = est_on_off_note_vels.astype(np.float64)
    ...
    def output_dict_to_detected_pedals(self, output_dict):
        ...
        if len(est_tuples) == 0:
            ...
            est_on_off = est_on_off.astype(np.float64)
            return est_on_off
...
def note_detection_with_onset_offset_regress(
    ...
    for i in range(onset_output.shape[0]):
        if onset_output[i] == 1:
            ...
            if bgn != -1:
                ...
                frame_disappear, offset_occur = -1, -1
            bgn = i

        if bgn != -1 and i > bgn:
            """If onset found, then search offset"""
            if frame_output[i] <= frame_threshold and frame_disappear == -1:
                """Frame disappear detected"""
                frame_disappear = i

            if offset_output[i] == 1 and offset_occur == -1:
                """Offset detected"""
                offset_occur = i

            if frame_disappear != -1:
                if (
                    offset_occur != -1
                    and offset_occur - bgn > frame_disappear - offset_occur
                ):
                ...
                bgn, frame_disappear, offset_occur = -1, -1, -1
    ...
    output_tuples.sort(key=pairOnset)
...
def pedal_detection_with_onset_offset_regress(
    ...
    for i in range(1, frame_output.shape[0]):
        ...
        if bgn != -1 and i > bgn:
            ...
            if frame_disappear != -1 and i - frame_disappear >= 10:
